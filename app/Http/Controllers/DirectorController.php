...
class DirectorController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        $downloads=Downloadpage::where('pagename','mot-du-directeur')->get()->map(function ($item){         
            if (App::isLocale('en')) {
           ...
           $tabs=Tabpage::where('pagename','mot-du-directeur')->with('tabimages')->get()->map(function ($item){         
            if (App::isLocale('en')) {
                $item->title=$item->title_en;
        ...
        return view('director')->with(['datas'=>$datas,'shareFacebook'=>$shareFacebook,'shareWhatsapp'=>$shareWhatsapp,'shareLinkedin'=>$shareLinkedin,'downloads'=>$downloads,'pagevideos'=>$pagevideos,'tabs'=>$tabs]);
    }
}